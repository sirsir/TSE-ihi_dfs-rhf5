...
#Region "Attribute"
    ...
    Protected m_intWorkType As Integer
...
#Region "Method"
    ...
    Public Function IsPlcConnect() As Boolean
        Try
            ...
            Dim intTemp As Integer = m_objPlc.ReadMemoryWordInteger(My.Settings.ReadDataMemoryType _
                                                                    , 5000 _
                                                                    , 1 _
                                                                    , OMRON.Compolet.SYSMAC.SysmacPlc.DataTypes.BCD)(0)
            'm_objLineSetting.ReadStatusAddress , m_objLineSetting.ReadStatusLength
    ...
    Public Function SendKeepAlive() As Boolean
        Try
            m_objPlc.WriteMemoryWordInteger(My.Settings.WriteLifeMemoryType _
                                            , 5002 _
                                            , {1} _
                                            , OMRON.Compolet.SYSMAC.SysmacPlc.DataTypes.BCD)
            'm_objLineSetting.WriteLifeAddress
            Return True
        Catch exCon As OMRON.Compolet.SYSMAC.SysmacIOException
            m_strErrorMessage = "PLC Time Out"
            Return False
    ...
    Public Function SendSyncTime() As Boolean
        Try
            ...
            m_objPlc.WriteMemoryWordSyncDateTime(My.Settings.WriteSyncMemoryType _
                                           , 5010 _
                                            , astrSyncFormat _
                                            , OMRON.Compolet.SYSMAC.SysmacPlc.DataTypes.BCD)
            'm_objLineSetting.WriteSyncAddress
    ...
    Protected Sub WriteErrorStatus(ByVal intErrorCode As Integer)
        Dim blnSent As Boolean = False
        While Not blnSent
            Try
                m_objPlc.WriteMemoryWordInteger(My.Settings.WriteStatusMemoryType _
                                                , My.Settings.WriteErrorStatusAddress _
                                                , {intErrorCode} _
                                                , OMRON.Compolet.SYSMAC.SysmacPlc.DataTypes.BCD)
                blnSent = True
                m_clsLogger.AppendLog(Me.GetType.Name, "WriteErrorStatus", "Write Error = " & intErrorCode & " at DM" & (My.Settings.WriteErrorStatusAddress), "Info")
            Catch exCon As OMRON.Compolet.SYSMAC.SysmacIOException
                m_clsLogger.AppendLog(Me.GetType.Name, "WriteErrorStatus", "Time Out", "Error")
            Catch ex As Exception
                m_clsLogger.AppendLog(Me.GetType.Name, "WriteErrorStatus", ex)
            End Try

            Me.m_objPlcLineThread.ReportProgress(0)
            Threading.Thread.Sleep(TimeSpan.FromMilliseconds(100))
        End While
        m_clsLogger.AppendLog(Me.GetType.Name, "WriteErrorStatus", "End Function", "Info")
    End Sub
